...
def _p(text, style):
    """Wrap Paragraph with XML escaping for ReportLab mini-XML parser."""
...
def render_resume(plan: DraftPlan, resume: Resume, startup_name: str, *,
    ...
    if ordered_skills:
        story.append(_p("Skills", h_sec))
        story.append(_p(", ".join(ordered_skills), body))

    if resume.education:
        story.append(_p("Education", h_sec))
        for ed in resume.education:
            line = " — ".join(b for b in (ed.school, ed.degree, ed.year) if b)
            story.append(_p(line, body))
            if ed.detail:
                story.append(_p(ed.detail, bullet))
